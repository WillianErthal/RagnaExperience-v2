...
void pc_battle_stats(map_session_data *sd, map_session_data *tsd, int32 flag)
{
	...
	if (flag == 1) {
		if (sd != tsd && tsd->status.bgstats.showstats) {
			clif_displaymessage(sd->fd, "O jogador não permite você ver suas informações.");
			return;
		}
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "-------------------------- INFORMAÇÕES DE BATTLEGROUND --------------------------", false, SELF);
		sprintf(output, "•   Nome: %s    -    Classe: %s", tsd->status.name,job_name(tsd->status.class_)); clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		if (pc_famerank(tsd->status.char_id,-1))
			sprintf(output, "•   Ranking: %d    -    Pontuação: %d", pc_famerank(tsd->status.char_id,-1),tsd->status.bgstats.points);
		else
			sprintf(output, "•   Ranking: N/A    -    Pontuação: %d",tsd->status.bgstats.points);
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		sprintf(output, "•   Vitórias: %d    -    Derrotas: %d    -    Empates: %d",tsd->status.bgstats.win,tsd->status.bgstats.lost,tsd->status.bgstats.tie);
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		sprintf(output, "•   Eliminações: %d    -    Mortes: %d", tsd->status.bgstats.kill_count, tsd->status.bgstats.death_count);
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		sprintf(output, "•   Dano Aplicado: %d    -    Maior Dano: %d", tsd->status.bgstats.damage_done,tsd->status.bgstats.top_damage);
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		sprintf(output, "•   Dano Recebido: %d    -    Cura Realizada: %d", tsd->status.bgstats.damage_received, tsd->status.bgstats.healing_done);
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		sprintf(output, "•   Modo Desertor: %d", tsd->status.bgstats.deserter);
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		sprintf(output, "•   Poções de HP: %d    -    Poções de SP: %d", tsd->status.bgstats.hp_heal_potions, tsd->status.bgstats.sp_heal_potions);
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		sprintf(output, "•   SP Gasto: %d", tsd->status.bgstats.sp_used);
		clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		if (tsd->status.bgstats.yellow_gemstones || tsd->status.bgstats.red_gemstones|| tsd->status.bgstats.blue_gemstones) {
			sprintf(output, "•   Gemas Amarelas: %d    -    Gemas Vermelhas: %d    -    Gemas Azul: %d", tsd->status.bgstats.yellow_gemstones, tsd->status.bgstats.red_gemstones, tsd->status.bgstats.blue_gemstones);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.poison_bottles) {
			sprintf(output, "•	Garrafas de Veneno: %d", tsd->status.bgstats.poison_bottles);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.zeny_used || tsd->status.bgstats.spiritb_used || tsd->status.bgstats.ammo_used) {
			sprintf(output, "•    Zenys Gastos: %d    -    Esféras Espírituais: %d    -    Munições: %d", tsd->status.bgstats.zeny_used, tsd->status.bgstats.spiritb_used, tsd->status.bgstats.ammo_used);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.leader_win || tsd->status.bgstats.leader_lost || tsd->status.bgstats.leader_tie) {
			sprintf(output, "•   Vitórias como Líder: %d   -   Derrotas como Líder: %d   -   Empates como Líder: %d", tsd->status.bgstats.leader_win, tsd->status.bgstats.leader_lost, tsd->status.bgstats.leader_tie);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.ti_wins || tsd->status.bgstats.ti_lost || tsd->status.bgstats.ti_tie) {
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "•   MODO: Santuário do Sacrifício -------------", false, SELF);
			sprintf(output, "•   Vitórias: %d    -    Derrotas: %d    -    Empates: %d", tsd->status.bgstats.ti_wins, tsd->status.bgstats.ti_lost, tsd->status.bgstats.ti_tie);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Pontos de Sacríficio: %d", tsd->status.bgstats.skulls);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.eos_wins || tsd->status.bgstats.eos_lost || tsd->status.bgstats.eos_tie) {
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "•   MODO: Olho da Tempestade -----------------", false, SELF);
			sprintf(output, "•   Vitórias: %d    -    Derrotas: %d    -    Empates: %d", tsd->status.bgstats.eos_wins, tsd->status.bgstats.eos_lost, tsd->status.bgstats.eos_tie);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Bandeiras: %d    -    Bases: %d", tsd->status.bgstats.eos_flags, tsd->status.bgstats.eos_bases);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.boss_wins || tsd->status.bgstats.boss_lost || tsd->status.bgstats.boss_tie) {
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "•   MODO: Bossnia ----------------------------", false, SELF);
			sprintf(output, "•   Vitórias: %d    -    Derrotas: %d    -    Empates: %d", tsd->status.bgstats.boss_wins, tsd->status.bgstats.boss_lost, tsd->status.bgstats.boss_tie);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Chefes Mortos: %d    -    Dano a Chefes: %d    -    Bandeiras: %d", tsd->status.bgstats.boss_killed, tsd->status.bgstats.eos_bases, tsd->status.bgstats.boss_flags);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.td_wins || tsd->status.bgstats.td_lost || tsd->status.bgstats.td_tie) {
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "•   MODO: Partida Rápida ----------------------", false, SELF);
			sprintf(output, "•   Vitórias: %d    -    Derrotas: %d    -    Empates: %d", tsd->status.bgstats.td_wins, tsd->status.bgstats.td_lost, tsd->status.bgstats.td_tie);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Eliminações: %d    -    Mortes: %d", tsd->status.bgstats.td_kills, tsd->status.bgstats.td_deaths);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.dom_wins || tsd->status.bgstats.dom_lost || tsd->status.bgstats.dom_tie) {
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "•   MODO: Dominação --------------------------", false, SELF);
			sprintf(output, "•   Vitórias: %d    -    Derrotas: %d    -    Empates: %d", tsd->status.bgstats.dom_wins, tsd->status.bgstats.dom_lost, tsd->status.bgstats.dom_tie);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Eliminações Ofensivas: %d    -    Eliminações Defensivas: %d", tsd->status.bgstats.dom_off_kills, tsd->status.bgstats.dom_def_kills);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		...
		if (tsd->status.bgstats.sc_wins || tsd->status.bgstats.sc_lost || tsd->status.bgstats.sc_tie) {
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "•   MODO: Controle de Barricada ------------", false, SELF);
			sprintf(output, "•   Vitórias: %d    -    Derrotas: %d    -    Empates: %d", tsd->status.bgstats.sc_wins, tsd->status.bgstats.sc_lost, tsd->status.bgstats.sc_tie);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Capturas: %d    -    Barricadas Capturadas: %d-    Barricadas Dropadas: %d", tsd->status.bgstats.sc_captured, tsd->status.bgstats.sc_stole, tsd->status.bgstats.sc_droped);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.ctf_wins || tsd->status.bgstats.ctf_lost || tsd->status.bgstats.ctf_tie) {
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "•   MODO: Capturar Bandeira ----------------", false, SELF);
			sprintf(output, "•   Vitórias: %d    -    Derrotas: %d    -    Empates: %d", tsd->status.bgstats.ctf_wins, tsd->status.bgstats.ctf_lost, tsd->status.bgstats.ctf_tie);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Bandeiras Capturadas: %d    -    Bandeiras Eliminadas: %d    -    Bandeiras Dropadas: %d", tsd->status.bgstats.ctf_captured, tsd->status.bgstats.ctf_taken, tsd->status.bgstats.ctf_droped);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.cq_wins || tsd->status.bgstats.cq_lost) {
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "•   MODO: Conquista --------------------------", false, SELF);
			sprintf(output, "•   Vitórias: %d    -    Derrotas: %d", tsd->status.bgstats.cq_wins, tsd->status.bgstats.cq_lost);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Eliminações Emperium: %d    -    Eliminações Baricadas: %d", tsd->status.bgstats.emperium_kill, tsd->status.bgstats.barricade_kill);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Eliminações Guardiões: %d", tsd->status.bgstats.gstone_kill);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
		if (tsd->status.bgstats.ru_wins || tsd->status.bgstats.ru_lost) {
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], "•   MODO: Rush --------------------------------", false, SELF);
			sprintf(output, "•   Vitórias: %d    -    Derrotas: %d", tsd->status.bgstats.ru_wins, tsd->status.bgstats.ru_lost);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
			sprintf(output, "•   Capturas: %d", tsd->status.bgstats.ru_captures);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		}
	// War of Emperium Stats
		...
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], "-------------------------- INFORMAÇÕES GUERRA DA EMPERIUM --------------------------", false, SELF);
		sprintf(output, "•   Nome: %s    -    Classe: %s", tsd->status.name,job_name(tsd->status.class_));
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
		if (pc_famerank(tsd->status.char_id,-2))
			sprintf(output, "•   Ranking: %d    -    Pontuação: %d", pc_famerank(tsd->status.char_id,-2),tsd->status.bgstats.points);
		else
			sprintf(output, "•   Ranking: N/A    -    Pontuação: %d",tsd->status.bgstats.points);
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
		sprintf(output, "•   Vitórias: %d    -    Derrotas: %d    -    Empates: %d",tsd->status.bgstats.win,tsd->status.bgstats.lost,tsd->status.bgstats.tie);
		sprintf(output, "•   Eliminações: %d    -    Mortes: %d", tsd->status.wstats.kill_count, tsd->status.wstats.death_count);
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
		sprintf(output, "•   Dano Aplicado: %d    -    Maior Dano: %d", tsd->status.wstats.damage_done,tsd->status.wstats.top_damage);
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
		sprintf(output, "•   Dano Recebido: %d    -    Cura Realizada: %d", tsd->status.wstats.damage_received, tsd->status.wstats.healing_done);
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
		sprintf(output, "•   Poções de HP: %d    -    Poções de SP: %d", tsd->status.wstats.hp_heal_potions, tsd->status.wstats.sp_heal_potions);
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
		sprintf(output, "•   SP Gasto: %d", tsd->status.wstats.sp_used);
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
		sprintf(output, "•   Eliminações Emperium: %d    -    Dano Emperium: %d", tsd->status.wstats.emperium_kill, tsd->status.wstats.emperium_damage);
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
		sprintf(output, "•   Eliminações Barricada: %d    -    Dano Baricada: %d", tsd->status.wstats.barricade_kill, tsd->status.wstats.barricade_damage);
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
		sprintf(output, "•   Eliminações Guardiões: %d    -    Dano Guardiões: %d", tsd->status.wstats.guardian_kill, tsd->status.wstats.guardian_damage);
		clif_messagecolor(&sd->bl, color_table[COLOR_CYAN], output, false, SELF);
...
bool pc_equipitem(map_session_data *sd,int16 n,int32 req_pos,bool equipswitch)
{
	...
	if (battle_config.button_sound_mode && sd->state.buttonsound == 1) {
	    if (pos & (EQP_ARMOR | EQP_HEAD_TOP | EQP_HEAD_MID | EQP_HEAD_LOW | EQP_SHOES | EQP_GARMENT | EQP_ACC_L | EQP_ACC_R)) {
	        clif_soundeffect(sd->bl, "equipequipments.wav",0,SELF);
	    } 
	    if (pos & EQP_HAND_R) {
	        clif_soundeffect(sd->bl, "equipweapon.wav", 0, SELF);
	    }
	    if (pos & EQP_HAND_L) {
	        if (id) {
	            if (id->type == IT_WEAPON) {
	                ...
	                clif_soundeffect(sd->bl, "equipshield.wav", 0, SELF);
	            }
	        }
	    }
	}
...
bool pc_unequipitem(map_session_data *sd, int32 n, int32 flag) {
	...
	if (battle_config.button_sound_mode && sd->state.buttonsound == 1) {
	    if (pos & (EQP_ARMOR | EQP_HEAD_TOP | EQP_HEAD_MID | EQP_HEAD_LOW | EQP_SHOES | EQP_GARMENT | EQP_ACC_L | EQP_ACC_R)) {
	        clif_soundeffect(sd->bl, "removeequipments.wav", 0, SELF);
	    } 
	    if (pos & (EQP_HAND_R | EQP_HAND_L)) {
	        clif_soundeffect(sd->bl, "removeweapon.wav", 0, SELF);
	    }
	}
	if(pos & EQP_HAND_R) {
		sd->weapontype1 = W_FIST;
...
***********************************************************/
int32 pc_update_last_action(map_session_data *sd)
{
	if (!sd->bg_id)
		return 1;
	std::shared_ptr<s_battleground_data> bg = util::umap_find(bg_team_db, sd->bg_id);
