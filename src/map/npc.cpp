...
#include "log.hpp"
#include "map.hpp"
#include "mapreg.hpp" //RAGNAEXPERIENCE - TUMBA
#include "mob.hpp"
#include "navi.hpp"
#include "pc.hpp"
#include "pet.hpp"
...
int32 npc_click(map_session_data* sd, struct npc_data* nd)
{
	...
	case NPCTYPE_SHOP:
		if (battle_config.button_sound_mode && sd->state.buttonsound == 1) {
			clif_soundeffect(sd->bl, "npc_click.wav", 0, SELF);
		}
...
int32 npc_cashshop_buylist(map_session_data* sd, int32 points, std::vector<s_npc_buy_list>& item_list) {
	...
	for (i = 0; i < item_list.size(); i++) {
		...
		if (!pet_create_egg(sd, nameid)) {
			...
			item_tmp.identify = 1;
			if (battle_config.button_sound_mode && sd->state.buttonsound == 1) {
				clif_soundeffect(sd->bl, "buyitem.wav", 0, SELF);
			}
...
static int32 npc_buylist_sub(map_session_data* sd, std::vector<s_npc_buy_list>& item_list, struct npc_data* nd) {
	...
	if (battle_config.button_sound_mode && sd->state.buttonsound == 1) {
		clif_soundeffect(sd->bl, "sellitem.wav", 0, SELF);
	}
