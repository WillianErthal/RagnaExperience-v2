...
void trade_tradeack(map_session_data *sd, int32 type)
{
	...
	if (type == 4) { // Cancel
		...
		if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
			clif_soundeffect(sd->bl,"cancel.wav",0,SELF);
		}
		return;
	}
	...
	tsd->state.trading = 1;
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
		clif_soundeffect(sd->bl,"oktrade.wav",0,SELF);
	}
...
void trade_tradeadditem(map_session_data *sd, int16 index, int16 amount)
{
	...
	if( trade_i == 10 ) { // No space left
		if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
			clif_soundeffect(sd->bl,"invalidtrade.wav",0,SELF);
		}
	...
	if( target_sd->weight + sd->deal.weight + trade_weight > target_sd->max_weight ) { // fail to add item -- the player was over weighted.
		...
		if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
			clif_soundeffect(sd->bl,"putitemtrade.wav",0,SELF);
		}
		return;
	}
...
void trade_tradecommit(map_session_data *sd)
{
	...
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
		clif_soundeffect(sd->bl,"completetrade.wav",0,SELF);
	}
