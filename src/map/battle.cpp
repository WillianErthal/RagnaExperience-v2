...
#else
			if (sc->getSCE(SC_SHRINK) && rnd() % 100 < 2 * sce->val1)
			...
			switch(skill_id) {
				...
				case MG_THUNDERSTORM:
					if(sc->getSCE(SC_GUST_OPTION))
						ad.damage += (6.5 + sstatus->int_ / 4) + max(sstatus->dex - 10, 0) / 30;
					break;
				case MG_FIREBOLT:
				case MG_FIREWALL:
...
//----------------------------------------------------
// A chance da habilidade ser ativada agora é 25% fixa
//----------------------------------------------------
...
// BattleGround Settings
	...
	{ "button_sound_mode",						&battle_config.button_sound_mode,					1,	0,	1,	},
// rAthena
